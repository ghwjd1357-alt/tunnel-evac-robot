...
### 0.5 사고 프로토콜 ★ (07-19 신설 — 모델 무관 적용. Fable 세션의 검증된 작업 패턴을 규칙화)

> 판단·수정·검증의 **순서**를 강제한다. 아래는 전부 07-19 검토 사이클에서 실효가 증명된 패턴.

1. **주장은 재현 후 수용** — 외부 검토·버그 보고·추측은 코드/메모리 재현부터. 재현 성공 = 즉시 반영, 재현 실패 = 기각하되 근거 기록. (재현 붙은 Codex 검토 반영률 100% vs 걱정만 있던 1차 검토 ~50% — 0719_현황.md)
2. **수정 전 완료판정 선정의** — "무엇이 관찰되면 성공인가"를 코드 작성 **전에** 한 문장으로. (마스터플랜 S표 '완료 판정' 열 형식)
3. **경계조건 질문 3종을 매 수정마다** — ① 타이머/디바운스가 있으면: "리셋은 언제 되나? 상태 전환(단절→복구) 순간은?" ② 비동기 호출이 있으면: "호출≠접수≠종결≠실효 — 4단계 중 어디까지 확인하나?" ③ 외부 입력이 있으면: "NaN/Inf/bool/음수/빈값이 오면?"
4. **완료 선언 전 자기 논증 반증 1회** — "이 논증이 틀리는 입력이 존재하나?"를 스스로 묻기. (§12.4 '런타임 None=화재≈탈출구뿐' 단정의 반례(동일 투영점)를 외부 검토가 찾아낸 사건이 교훈 — 반증은 남이 하기 전에 내가)
5. **검사를 조이면 역회귀 앵커 동반** — 검증 강화 시 "실배포 설정 2종(T자·쌍굴)이 여전히 통과"하는 테스트를 같이 추가 (멀쩡한 설정을 죽이는 역회귀 방지).
6. **테스트 실패는 '코드 vs 인프라' 분류 후 대응** — E2E 실패 시 로그 물증으로 기지 함정(응답 유실·좀비 bt_navigator·데몬 오류)과 대조. 인프라면 재시도+스크립트 방어 추가, 코드면 수정. **원인 한 줄 기록 전에 재실행 금지** (그냥 재실행해서 통과하면 flake 가 영구 은폐됨).
7. **성공 경로만 보는 테스트는 미완성** — "안 돼야 하는 게 안 되는가"(부정 회귀)와 "전환 순간"(복구 첫 프레임 등) 케이스까지 있어야 완성.
8. **내린 결정은 재논의 금지** — 마스터플랜·현황.md 에 근거와 함께 기록된 정책 결정은 새 근거 없이 뒤집지 않는다. 새 근거가 생기면 해당 문서를 개정하고 개정 사유를 남긴다.
9. **작업 단위는 잘게, 회귀는 즉시** — 한 턴에 한 묶음(S표 행 단위) 이하. 변경 직후 pytest(0.2s)는 무조건, E2E 는 해당 영역 것만.
...
| **테스트·회귀 인프라** | ✅ **2026-07-20 갱신:** pytest **96개** + colcon **102 tests(실패 0, skip 2)** + E2E 4종(`regression_negative`·`regression_3goals`·`mission_e2e`·`abort_e2e`) + 지도 승격 격리 하네스 8검사. platform-core 구조 분리는 단계마다 전량 회귀(아래 현재 작업 지시·0719_현황 §18.4) |
...
### 로드맵 & 현재 위치
**2A** 클코 학습 ✅ → **2B** ROS2 기초 ✅ → **2C Gazebo ✅** → **2D URDF(differential) ✅** → **2E SLAM+Nav2+EKF ✅** → **2F 미션 로직 ✅ 시뮬 완성** → **G 안전봉합·녹색 기준선 ✅** → **ⓒ 중간보고서 ✅ 완료** → ◀ **현재 = platform-core 구조 분리 1/3, SpeedManager 추출**. 순서 정본: `0719_실차전환_마스터플랜.md §7.2~7.3`, 즉 SpeedManager → GoalManager → E2E 공통 하네스 → Codex 게이트 → platform-core-freeze.
...
- ✅ **S1 미션 신뢰경계 (07-19 심야 — 마스터플랜 §2 첫 묶음)** — Codex §8.2·§9.2·§9.4·§10.6 재현 전량 방어. ① **/alarm 진입 검증**: 유한값·frame=map·★그래프 투영거리 상한 `alarm_max_projection_dist`(yaml 신설, 5m) — NaN 화재가 집결지 (4,0)로 둔갑·1km 오클릭이 정상 접수되던 구멍, 거부 시 사유+다음 행동 로그. ② ★**동일 투영점 정책 확정(§12.4 결정 개정)**: 그래프 선언 시 직선 fallback 폐기 — 그래프 계산 실패(화재≈탈출구/동일 투영점)면 yaml 검증 고정 집결지 직행 (직선식은 그래프 미선언 구성에서만). ③ validate_waypoints 필수·선택 키 숫자·유한·부호 검증(정수·상한·상호관계는 F6 에서 추가)(`need_num` — patrol NaN·음수 대기시간 등 공격 4종 검거, 속도는 양수 강제). ④ Action 콜백 `future.result()`/cancel 예외 방어 — 전파되면 goal_active=True 영구 대기이던 것을 FAULT 정리로. ⑤ set_nav_speed 응답 `successful` 확인+3회 재시도 (★비동기는 '호출' 아닌 '완결' 검증 — cancel 감시와 동일 원칙) + mission_e2e 에 GUIDE 0.12 실측 assert. ⑥ scan `header.stamp` 단조성 — 죽은 드라이버의 동일 프레임 재전송을 신선으로 오인 방지. ⑦ 지난 goal 최종 status 관찰(CANCELED 확인 사슬 마지막 조각). pytest 56→**81**(알람 경계 전용 `test_alarm_boundary.py` 신설 — `__new__` 기법을 on_alarm 로 확장). 정본 = **0719_현황.md §13**, 다음 = S2(도구 안전, 마스터플랜 §2)
- ✅ **S2 도구 안전 + S3 문서·게이트 (07-19 심야 2차 — 시뮬 마감 묶음 완결)** — ① ★make_map **지도 생명주기**: 모드 엄격(`tunnel|twin` 외 즉사, `twim` 오타 방어)·**staging→localization 스모크→정본 승격**(`.bak` 백업+manifest: commit·SHA-256). 첫 실행이 **정책 충돌 검거**: 07-19 운영 정책(track_unknown_space:true+allow_unknown:false)은 mapping 탐사와 양립 불가(미지 공간 계획 거부) → slam_nav2.launch `nav2_params` 인자 신설 + mapping 한정 `track_unknown_space:false` 오버레이(07-06 지도 v3 를 성공시킨 검증 구성 복원. ⚠ allow_unknown:true 우회는 NavFn "legal potential" 결함 실측 기각). ★교훈: 정본 설정은 운영 기준, 특수 세션은 명시 오버레이 / 회귀에 없는 모드가 정책 변경의 사각지대. 부수 성과 — 실패 순간 staging 이 정본 무손상을 실전 증명. ② 벤치 기본 localization(mapping 수치 오인 방지)+메타·라벨 sanitize, report CSV/라벨 개수 불일치 에러(zip 조용한 누락 종결)·NaN 행 검거 ③ 셸 6종 `trap INT TERM`+★전용 시뮬 PC 전용 명시(Jetson 실행 금지 — 전면 격리는 과투자 기각) ④ abort_e2e 문구 정합·콘솔 RTT 타이머 누수·fake_follower 콜백 방어 ⑤ S3: 명칭 제한("근거리 물체 추종감시")·정확도 시뮬 상한 명시·`regression_3goals` 허용치 **0.3m**·로드맵 P0 병합. 검증: 새 정본 지도로 mission_e2e 재완주(지도 생명주기 이중 검증)·abort PASS. **★Codex 최종 재검토 인계장 = 0719_현황.md §15** (변경 파일 표·재검토 포인트 7·재실행 테스트)
...
- **★ platform-core 구조 분리 단계 전체 게이트 (07-20):** `python3 -m pytest src/mission_manager/test/ -q` → `colcon test --packages-select mission_manager tunnel_sim && colcon test-result --verbose` → `bash tools/regression_negative.sh` → `bash tools/regression_3goals.sh` → `bash tools/mission_e2e.sh` → `bash tools/abort_e2e.sh`. 각 프로세스 완전 종료 후 다음 실행. 지도 자산을 바꾸는 `make_map.sh`는 구조 분리 검증에 포함하지 않는다.
- **부정 회귀 (07-19 신설 — 안전 정책·Nav2 설정 변경 시 + ★정본 지도 '수동' 교체 시 추가 실행. T자 make_map 은 G3(07-20)부터 승격 전 staging 지도로 **자동 실행**되므로 별도 불필요 — F1 수락 기준의 자동화):** `bash tools/regression_negative.sh`(~6분) — 지도밖/벽너머/막힌 goal 이 **실패 종결**해야 PASS(성공하면 안전 정책 구멍, timeout 이면 무한 회복 의심) + 정상 goal 양성 대조군. 실측: 막힌 goal 은 BT 재시도 6회 소진까지 **~2분 걸려 ABORTED**(무한 아님 — planner tolerance 백로그의 판단 재료). ⚠ `service call` 응답 파싱은 `success=True`(repr 형식, `success: True` 아님). (0719_현황.md §8)
