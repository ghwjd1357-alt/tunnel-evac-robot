...
- ▶ **다음 (정본 = 마스터플랜 §7.2 순서 12단계):** ③ ★ⓒ 중간보고서 초안(마감 有, 재료·그래프 확보 완료) → ④ 구조 분리(SpeedManager → GoalManager → E2E 하네스, 단계마다 회귀 전량) → ⑤ platform-core-freeze. 병렬: ⓑ 역할 B **V1 최소 계약** 미팅(책임경계 택1: 2D bbox / camera 3D — §7.4 안건) / ⓔ micro-ROS(구동부 대기). 보류: 감지 3차·BT BackUp(시나리오 확정 후 재평가). + 학습: **정본 = `0707_학습로드맵.md`** — 다음 계단 = 1단계 URDF 코드리딩.
