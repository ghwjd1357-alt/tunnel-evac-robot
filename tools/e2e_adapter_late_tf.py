#!/usr/bin/env python3
"""늦은 TF **지연 도착 형상** 검사 (08-21 §85.2 · 문구 정정 §86.6).
...
⚠ 이 검사가 증명하지 않는 것 — 🔴 **범위를 과장하지 않는다**(§86.6 지적 수용):
  · Jetson 에서의 실제 지연 분포. `tf_wait_sec` 값 근거는 여전히 약하다(§84.1 미이행)
  · **package `main()` 자체는 안 부른다** — executor·구독 topology 만 같게 맞췄다
  · launch remap · 기동 예외 · 종료 경로는 안 본다
  · 변이(리스너를 어댑터 노드로)에서 5/5 → 3/5 로 죽는 것은 확인했지만
    그 변이는 **자동 회귀가 아니다** — 사람이 손으로 넣어 본 1회 관측이다
  ⇒ 그래서 이 파일이 증명하는 것은 *"지연 TF 형상"* 이지 *"실제 entrypoint 전체"* 가
    아니다. 구판 머리말이 후자로 적혀 있었고 그것은 과장이었다.
"""
