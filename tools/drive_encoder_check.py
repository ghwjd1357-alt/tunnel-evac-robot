...
def analyze(rows, groups, wheel_base_m=None, wheel_radius_m=None):
    ...
    if len(groups) != 4:
        return {'ok': False,
                'reason': f'굴림 구간이 {len(groups)}개다 — 네 바퀴를 한 번에 하나씩, '
                          f'사이에 {QUIET_S}s 이상 정지하며 굴린다'}
    ...
    verdicts, bad = [], []
    for i, (row, want) in enumerate(zip(out, EXPECTED_SIGNS)):
...
def report(v):
    ...
    for i, (r, name, verdict) in enumerate(zip(v['rows'], WHEEL_NAMES, v['verdicts']), 0):
        mark = {'ok': '✅ 정상',
                'flipped': '🔴 부호 반전',
                'dead': '🔴 반응 없음'}[verdict]
    ...
    print()
    if v['ok']:
        print('  ✅ 좌측 2개 음수 · 우측 2개 양수 — 부호 배치가 정상이다.')
    else:
        names = ', '.join(WHEEL_NAMES[i] for i in v['bad'])
        print(f'  🔴 **{names}** 가 기대 부호가 아니다.')
