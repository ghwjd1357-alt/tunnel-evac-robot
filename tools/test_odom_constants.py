...
#:   🔴 **08-14 정정 (검토 §71.1) — "0.83% 라 R2 가 안 뒤집힌다" 는 틀렸다. 내렸다.**
#:     세 시행이 요구하는 base 가 0.82218~0.82902 로 0.83% 안에 모이는 것은 **반복
#:     일관성**이지 정확도 상한이 아니다. 세 시행이 **같은 AMG 적분 경로**를 지나므로
#:     그 경로의 **공통 scale/bias 오차는 세 값을 함께 옮겨** 스프레드에 안 나타난다.
#:     즉 실제 base 가 세 역산값 **밖일 가능성**이 그 논증에서 빠져 있었다.
#:   → 이 screen 을 굽기 전에 닫지 않는 근거는 정량 논증이 아니라 **절차**다:
#:     R2 회전의 최종 권위가 **굽기 뒤 지면 2π** 이고, 그것이 `324~396°` 를 벗어나면
#:     **즉시 실패·중단**하고 AMG 공통모드 오차를 실측한다(검토 §71.7 진행 경계).
