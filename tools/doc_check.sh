...
# 핸드오프 완료조건·완료판정은 같은 행 수를 정확히 한 번씩 말해야 한다.
# 08-17 §73: 1.6.1은 축소 6행 승인 지문 밖이므로 현행 계약은 13행 전량이다.
# 역사 서술은 오탐이 아니므로 현재 계약을 적는 두 절만 본다. 양쪽 경계는 exact-one으로
# 잠가 자리 소실뿐 아니라 같은 주장의 증식도 실패시킨다.
HANDOFF="docs/CURRENT_HANDOFF.md"
ROWS_ACTIVE=$(awk '/^## 완료조건/,/^## 완료 판정/' "$HANDOFF" | grep -c '13행 전량 수행')
ROWS_VERDICT=$(awk '/^## 완료 판정/,/^## 완료 후 다음 단계/' "$HANDOFF" | grep -c '13행 전량 수행')
ROWS_STALE=$(awk '/^## 완료조건/,/^## 완료 후 다음 단계/' "$HANDOFF" \
             | grep -c '§7-c-E2\|배선 6행\|§7-c-E\*\* 13/13\|`§7-c-E` 13/13')
if [ "$ROWS_ACTIVE" -eq 1 ] && [ "$ROWS_VERDICT" -eq 1 ] && [ "$ROWS_STALE" -eq 0 ]; then
    ok "§7-c-E 행 수 계약 단일 (완료조건·완료 판정 둘 다 13행 전량)"
else
    bad "§7-c-E 행 수 계약이 갈렸다 — 완료조건13 $ROWS_ACTIVE · 완료판정13 $ROWS_VERDICT · 구판표현 $ROWS_STALE"
fi
