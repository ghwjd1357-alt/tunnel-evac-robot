...
class FollowerMonitor:
    ...
    def reset(self, zone='rear'):
        ...
        · `[reset-role] hold-return` — 🆕 08-22 HOLD(제자리 재수집) 에서 복귀.
          놓침 확정 뒤 **역행하지 않고** 서서 기다리다 다시 보인 경우다.
          안 하면 `lost` 타이머가 놓침 당시 그대로라 복귀 첫 tick 에 다시 lost 가
          참이 되어, 아낀 역행이 결국 나간다 — HOLD 를 만든 이유가 지워진다.
