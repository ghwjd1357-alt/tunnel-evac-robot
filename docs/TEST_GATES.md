...
→ 변경 파일을 **런치 인자로 명시**해 돌린다:

```
ros2 launch tunnel_sim slam_nav2.launch.py nav2_params:=nav2_params_real_db.yaml
```
