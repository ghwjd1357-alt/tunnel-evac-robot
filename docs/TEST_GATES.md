...
| **판정 도구** (`d0_check.sh` · `bag_gap_report.py` · `todo_d0_scan.py` — 실차/문서 판정기) | 없음 (E2E 와 인과 없음). 대신 **그 판정기 자신을 공격**한다: `bash -n` / `py_compile` / `python3 -m flake8 --max-line-length=99`(ament 규약) / `scan_unbounded_cli` / 합성 입력 부정·역회귀. ★ **08-03 §30 신설** — 이 도구들은 `colcon test` 밖에 있어 **회귀가 저장소에 없다**(예약 21). 그래서 검토자는 기록된 명령을 **직접 재실행**하고, 실행 증거에 **명령이 적혀 있는지**부터 본다(§30.5 가 정확히 그 자리에서 났다) |
