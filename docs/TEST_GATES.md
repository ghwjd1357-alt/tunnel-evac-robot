...
| **AI 입력·라우터** (`AGENTS.md` · `CLAUDE.md` · `CURRENT_HANDOFF.md` · `AI_CONTEXT.md` · `ai_context.py` · 알려진 P0/P1 목록/테스트) | E2E 없음. `python3 tools/test_ai_context.py` 전량 + `py_compile` + ament flake8. 미지·혼합·고위험 fallback, 역할별 게이트, diff 무손실, 예약 23 비접촉, cold-start 30% 하한을 공격한다. 알려진 59건의 **라우팅 회수율**과 실제 모델 재검출률은 구분한다. raw input은 원문을 외부로 보내지 않는 `node tools/local_token_count.js`(o200k)로 비교하고, 외부 모델 JSON usage는 별도 승인 때만 보조 측정한다 |
