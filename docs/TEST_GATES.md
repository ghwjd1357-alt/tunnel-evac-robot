...
bash tools/test_harness_guards.sh                        # ~125초 — 하네스 유한 상한 + 외부 증거 파서 fail-closed 계약(§14~§16 P1 · 07-30 예약 4 · 07-31 §7·§8 P1)
...
| test_harness_guards | E2E 하네스 유한 상한 (`read_param_float` 복구 상한·`wait_state` 벽시계 deadline·**SIGTERM 무시도 hard-kill**·daemon kick 남은-예산 배분·mission topic-pub wiring) + **실정지 실패 원인 분류**(케이스 11·12)와 **`gz model` 유한 상한**(케이스 13) — Gazebo 불필요 | **17 케이스** 전부 ✓ (§14~§16 P1 · 07-30 예약 4 · 07-31 §7·§8 P1) |
...
| ↳ **판독기 fail-closed 계약** (07-31 §7.2 P1) | 07-30 판은 정규식에 걸린 값이 **하나도 없어도 `0건`** 을 찍어 빈·경고문뿐·필드누락·NaN/Inf 덤프가 전부 '잔류 없음'으로 둔갑했고 ⑧ 에서 **그대로 PASS** 였다. 이제 **완전한 Twist 표본(6성분 전부 유한)** 을 최소 1개 확인해야 정수를 반환한다. ⑦·⑧ 은 **단일 계약** `measure_cmdvel_residual` 만 소비한다 | 손상 입력은 전부 '판독 실패'. **꼬리 잘린 마지막 샘플은 손상이 아니다**(시간상자 수집의 정상 형상) |
| ↳ **구조 검증** (07-31 §8.2 P1) | 07-31 1차 보완은 "6성분"을 **"6줄"** 로 셌다 → `angular.{x,y,y}`(z 누락+y 중복)·`linear` 안의 6줄·섹션 밖 6줄이 전부 '완전한 Twist' 로 통과했다. 또 `seen_lines==0`(값 줄 수)을 '파일이 비었다'로 읽어 **경고문·쓰레기 텍스트 + 발행자 1** 이 '잔류 0건' 으로 승격됐다. → **키 집합**(`linear`·`angular` 각각 `{x,y,z}` 정확히 한 번씩)으로 판정하고, 침묵 예외는 `txt.strip()==""` 일 때만 | 섹션 중복·섹션 밖 x/y/z·키 중복·비숫자·NaN/Inf = 손상. **불완전 레코드는 마지막 하나만 허용**(잘림은 원리상 꼬리에만 생긴다) |
| ↳ ★ **'관측된 침묵'** (07-31 실측) | 이 시스템에서 abort 뒤의 '잠잠'은 zero Twist 가 아니라 **완전한 침묵**이다 — nav2 가 취소 후 발행 자체를 멈춰 **실덤프가 0바이트**다. "빈 덤프=무조건 판독 실패"로 두면 `abort_e2e` 가 **영구 거짓 FAIL** 이 된다. 그래서 침묵은 **관측 근거**(수집이 시간상자 정상 소진 + 그 시점 `/cmd_vel` **발행자 ≥1**)가 있을 때만 0건으로 인정한다 | 빈 덤프+발행자≥1 = **0건(PASS)** / 빈 덤프+발행자 0·조회실패 = **판독 실패** / 내용은 왔는데 온전한 표본 0개 = **판독 실패**. ⚠ `ros2 topic info` 는 daemon 의존 → 복구 1회 후에도 못 읽으면 fail-closed |
