...
| ↳ ★ **`&` 한 글자에 네 가지 뜻이 섞여 있었다** (07-31 §14.2 P1) | 배경 실행을 검사에서 빼는 근거는 오직 **'블록하지 않는다'** 인데, 구판은 앞의 `>`(`2>&1`)만 예외로 두고 **뒤의 `>`** 를 보지 않았다. 그래서 Bash 의 foreground 리다이렉션 `ros2 … &>/dev/null` · `gz … &>>log` 가 배경 실행으로 분류돼 **단위 전체가 검사에서 사라졌다** — 리다이렉션 표기 하나로 무상한 CLI 가 증발한다. 게이트 5파일엔 오늘 `&>` 가 0곳이라 런타임 결함은 아니지만, 이 검사기는 07-30 의 11분 행을 막는 **미래 회귀 안전망**이다 | 역할 판정을 `_amp_role()` **한 곳**으로 모아 `&`(배경) · `&&`(목록) · `&>`·`&>>`(리다이렉션) · `>&`(리다이렉션 일부)를 명시적으로 가른다. 케이스 19 픽스처가 **양성 22·음성 13** 으로 넓어졌다(양성 20·21 = `&>`·`&>>`, 음성 11·12·13 = 진짜 background·guarded `&>`·`2>&1`) |
...
- ★ **상한 집행은 벽시계(`SECONDS`)로, hard-kill 로 실제 보장한다** (07-24 §14·§15 P1 보완):
  ...
  - 이 부정·역회귀는 `tools/test_harness_guards.sh` **21케이스**가 Gazebo 없이 격리 검증한다
    ...
    **case 18=증거 수집이 발행자 조회보다 먼저(§10.5 P2-①)·case 19=게이트 5파일의 상한 없는
    외부 CLI 호출 전수 검사 + 검사기 자체의 **양성22/음성13** 부정 회귀(예약 17·§11.2·§12·§13·§14.2 P1)·
