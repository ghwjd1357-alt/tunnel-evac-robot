...
기준선 (**08-01 재갱신** — 예약 18 회귀 11건 편입 뒤 **§21 P2 2건 보완**으로 20건이 됨): pytest **159 passed** / colcon **208 tests, 0 fail, 3 skip** / **test_harness_guards 24 검사** / test_gate_regression **14 케이스** / E2E 4종 PASS **+ 쌍굴 PASS**.
...
| ↳ **가짜 센서 주기 회귀** (예약 18 · **§21 P2 보완**) | 게이트 회귀의 **입력원**이 실물과 같은 입력을 만드는가. ① IMU 주기 열이 실측 5통계(평균 24.02ms·min 18·max 30·σ 0.55·창 295)를 재현하고 **등간격으로 되돌리면 깨진다**(순수 계산) ② **실제 rclpy 타이머**로 생산 `FakeSensors` 를 돌려 `_imu()`→`_advance_imu_period()`→setter 사슬을 통과한 콜백 간격이 계획 열과 같은지 본다 ③ 주기 값을 만드는 **모든 정의가 '주기 정본' 블록 안에 닫혀 있는지**를 AST 의존 폐포로 본다(표기 무관) ④ 실측을 못 받은 `/scan`·`/odom` 은 **'미확보' 선언** 강제 | 20건 전부 ✓ (`colcon test` 안에서 ~2.1초). ⚠ 상한: 실측 295표본을 실시간으로 재현하지 않고 **짧은 시험 계획**(30/90/50/140ms, 되감기 포함)으로 배선을 잠근다 — "실측 최대가 실제로 발행된다"는 ①(계획이 최대를 담음)+②(계획대로 나옴)의 **합성**이다. 관측은 콜백 진입 시각이라 DDS 전송 지연은 안 섞이지만, 실제 발행 열에는 파이썬·DDS 지터 ±4ms 가 별도로 얹힌다 |
