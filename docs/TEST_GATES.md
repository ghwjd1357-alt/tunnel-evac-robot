...
🔴 **08-22 (§87.10) — Nav2 파라미터를 바꿨으면 그 파일로 회귀를 돌려야 한다.**
표준 `regression_3goals.sh`·`regression_negative.sh` 는 `tunnel_sim` 의 **기본**
`nav2_params.yaml`(SimpleProgressChecker·0.15)을 읽는다. 즉 `nav2_params_real_db.yaml`
을 바꾸고 표준 명령만 돌리면 **바꾼 파일을 한 줄도 검증하지 않은 초록**이 뜬다.
🔴 **08-22 §88.7 보강 — 위 명령만으로는 안 된다.** 런치를 띄우기만 하고 **goal 을
하나도 안 보낸다.** 판정을 실행하는 것은 두 회귀 스크립트이고, 둘 다 인자를 그대로
런치에 넘긴다(`EXTRA_ARGS=("$@")`). **복사 실행 가능한 명령은 이 둘이다:**

```
bash tools/regression_negative.sh nav2_params:=nav2_params_real_db.yaml
```

```
bash tools/regression_3goals.sh nav2_params:=nav2_params_real_db.yaml
```

⚠ 인자를 빼면 `tunnel_sim` 의 **기본** `nav2_params.yaml`(SimpleProgressChecker·0.15)
을 읽어, **바꾼 파일을 한 줄도 검증하지 않은 초록**이 뜬다. 그 상태를 "명시 실행"
으로 오해하지 않도록 **로그에서 실제 params 경로를 확인**한다:

```
grep -m1 nav2_params /tmp/*/launch.log
```

🔵 sim 기본 구성 회귀는 인자 없이 그대로 둔다 — 둘 다 필요하다.

⚠ 검토자가 명시 실행으로 재확인한 실측(08-22): 3-goal 전부 SUCCEEDED · 최종 오차
**0.236 m ≤ 0.3 m**, negative 는 지도밖·벽너머·막힌 goal 전부 ABORTED · 정상 대조
**0.218 m**. 표준 실행값(0.138 / 0.085)과 **다르다** — 그 차이가 검증의 내용이다.

기준선 (**08-02 재갱신 4** — 위 34건 위에 **예약 16 회귀 23건**(`test_search_back_entry.py` — SEARCH_BACK 진입 봉인 11 + 검토 §26 의 **GUIDE 진입 세대** 8 + 검토 §27 의 **센서 관측 세대** 4) 편입 · **08-12 예약 20 `ast` 전환 +2** · **08-23 §91 P0-2 재발견취소 직렬화 기구 +2 · 호출자리 +2 · P0-3 scan사망중 GUIDE 거동고정 +2 · 2회차 동기취소실패 +2 · 3회차 P0-1 늦은취소실패 +3 · 4회차 정지통보 경계 +5**): pytest **285 passed** / colcon **477 tests, 0 fail, 3 skip**(08-23 §91 P2-1 `test_lidar_standalone_equivalence` +3)(08-19 `tunnel_interfaces` 편입 +5) / **test_harness_guards 24 검사** / test_gate_regression **14 케이스** / E2E 4종 PASS **+ 쌍굴 PASS**.
...
역회귀를 더해 `bash tools/rearm_gate_host_test.sh` **동작 989건 + 구조 11건**이다.
현재 수치·해독 계약은 `REAL_ROBOT_VALUES §1-f·§1-g`가 소유한다.
⚠ 같은 회차에 배운 것 하나 더 — **`tools/**`·`docs/**` 를 검토에 보내지 않는다**
(`AGENTS §6`). 08-13 밤에 Tier B 커밋 3개를 보내 **P1 11 건을 스스로 만들었다.**
회차 계수기(`Review-round:`)를 안 남긴 것이 상한이 안 걸린 직접 원인이다.
...
`tools/drive_ground_report.py` — 🔴 **명령-실측 일관성 screen** 신설(§68.5 — 줄자 타당성의
증명이 아니라 성급한 상수 변경을 멈추는 넓은 경보다). `줄자 ÷ 시간` 이 명령 속도의
`0.85~1.30` 배를 벗어나면 *"이 값으로 상수를 바꾸지 마라"* 를 찍는다. 08-13 오후 값(0.77배)은
걸리고 밤 값(1.02배)은 통과한다 — **이 검사 한 줄이 하루를 막았을 것이다.**
`tools/rearm_field_wiring.py` — `§7-c-E` 축소판(배선 6 행 · 약 8 분). 🔴 두 헤더 **와**
생략 행이 지나는 `.ino` 함수 **9 개**의 sha256 이 다르면 **거부하고 13 행 전량으로
돌려보낸다**(§68.3·§69.3 — 앞 판은 헤더만 잠가 `driveDiagMessage.y` 나 `setup()` binding
변조를 통과시켰다). 거부 경로 회귀 = `python3 tools/test_wiring_gate.py`.
`tools/test_identity_schema.py` — `/firmware/info` 필수 스키마가 조용히 줄지 않는지.
**명시 shortlist 15 키**를 필수로 소유하고(§69.2·§71.2 — "전량" 이 아니다), 하나라도 format+인자에서
사라지면 거부한다. 런타임 카운터 제거는 통과. 코드 목록 ↔ 정본 표를 **양방향** 대조한다.
...
**불일치해야** 통과하는 필수 부정 회귀다. preset 이 `profile` 을 소유하므로 `--pre-plate`
를 잊어도 역사 값이 재현되고, preset 과 플래그가 모순되면 `rc=2` 다.
...
⑤ 🔴 **`T0` *앞* 결측을 봐주는가**(§58.1 — 바퀴 중심이 **누적 상태**라 T0 앞 실패가 그 뒤
전부를 오염시킨다. 중심 갱신을 건너뛰는 생산자 세 경로를 `frame_range[0]`·`T0-1`·T0 앞 연속
구간에 주입하고 **뒤 행을 전부 유한하게 둬도** 판정 불능이어야 한다. "나중 행이 finite" 는
상태 회복의 대리값이 아니다) ⑥ 정상 시행에서 `관측 완전성:` 줄이 실제로 인쇄되는가.
