...
- **★ 선행 보완 (재검토 1회 대기 — 이 묶음 착수 전에 확인할 것)**: 문서 동기화 커밋 `3faee22` 가
  `CODEX 현황/0723검토현황.md §10` 에서 **P2 1건 조건부 불승인**을 받았다 — 영구 증거가
  묶음마다 교체되는 작업 정본의 내용을 근거로 인용한 **참조 수명주기 결함**.
  보완 = 참조 2곳을 불변 역사·커밋 고정으로 교체 + `tools/doc_check.sh` 에 정적 프로브 신설.
  기록 = `0723_현황.md §13`. ⚠ 이 보완은 **직전 묶음 소관**이라 아래 허용 파일 밖
  (`tools/doc_check.sh`)을 의도적으로 건드렸다.
