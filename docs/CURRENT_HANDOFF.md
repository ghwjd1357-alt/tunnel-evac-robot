...
| 2 | **41-g 사건별 MCU 계측** | 🔶 **계측 계약만 확정**. 구현은 1.6.3 굽기가 있어야 하므로 `MASTER_PLAN §7` 예약 41-g의 완료판정에 넣고 `docs/PLAN_0818.md §3`에 시간표로 뺐다 |
...
2. 08-18 실행 순서는 `docs/PLAN_0818.md` — 1.6.3 굽기와 R3 재시험이 그 안에 있다.
...
| 트랙 | 묶음/근거 | 상태 |
|---|---|---|
| **진행 중 묶음** | §77 보완(예약 46 도구 + 41-g 계약 + `PLAN_0818`) | ✅ 구현 완료 · 🔴 독립검토 대기 |
