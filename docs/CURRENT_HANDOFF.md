...
- **★ 선행 보완 — 종결됨 (이 묶음 착수 전 배경)**: 문서 동기화 커밋 `3faee22` 가
  `CODEX 현황/0723검토현황.md §10` 에서 P2 1건(영구 증거가 가변 작업 정본의 내용을 인용한
  **참조 수명주기 결함**)을 받아 보완했고, **`CODEX 현황/0723검토현황.md §11` 재검토에서
  종결**됐다 (P0/P1 0건, 동결 태그·기준점 재개방 없음).
  재검토가 새로 지적한 **프로브 자체의 경계 결함**(줄바꿈 우회)도 이어서 보완했다 — Codex 가
  1회 규칙으로 루프를 닫았으므로 **추가 검토 요청 없이 종결**한다.
  기록 = `0723_현황.md §13`·`0723_현황.md §14`.
  ⚠ 두 보완은 **직전 묶음 소관**이라 아래 허용 파일 밖(`tools/doc_check.sh`)을 의도적으로 건드렸다.
