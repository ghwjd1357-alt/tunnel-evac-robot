...
- 🔴 **커널 층은 이번에도 미관측이다** (§77.2 정정). `dmesg_fw162_ground_soak_0817_2320.log`의
  ...
  **23:31:08.086613**보다 753.20초 이르므로 원본 조합은 rc=1이다. 단, 현 도구는 마지막
  사건을 끝 경계로 오인하므로 조용한 유효 로그도 거짓 거부할 수 있다(예약 46). 게다가 로그의
...
다음 구현자(Claude)는 한 묶음에서 아래 경계를 닫는다.

1. 예약 46: dmesg 취득 경계를 사건 시각과 분리하고, 시작·종료 exact 및 ±1초,
   조용한 사후 스냅샷, 메타데이터 소실 rc=2를 회귀로 고정한다.
2. 예약 41-g: 7개 공백마다 MCU 사건 시각을 대응시켜 loop 내부 47ms·판 사이 300ms·
   host/agent 300ms가 서로 다른 관측으로 갈리는 계측 계약을 세운다.
3. 예약 44: 스페어 교체 뒤 같은 시행의 좌우 회전수와 독립 yaw/궤적으로 완만한 곡선
   실효 윤거를 구한다. 0.829m나 6.24%·3.13%p를 선결론으로 넣지 않는다.
4. 포트 계수 4회와 진행표 41-g·44·46을 정본 전부에서 맞춘다.
5. `TODO(D+0)` **11건**과 `§7-c-E` **13행 전량 수행** 상태를 유지하고 기존 회귀
   기준선을 축소하지 않는다.
...
현 상태는 **§77 독립검토 완료, P1 보완 미완료**다. 두 대상 커밋과 현 보드 운용은
조건부 수용하지만 1.6.2 승인 지문은 이관하지 않는다. 수 초 감시-loop 정지의 재발 0은
현장 관측 범위에 한정하며, 349ms 단기 결측 때문에 R3 완료판정은 FAIL이다.
`TODO(D+0)` **11건**과 `§7-c-E` **13행 전량 수행** 계약은 그대로다.

## 완료 후 다음 단계

1. **§77의 세 P1**을 예약 46·41-g·44의 완료판정으로 보완한다.
2. 보완 뒤 승인 지문 이관 여부를 구현자와 다른 검토자가 결정한다.
3. 별도 묶음으로 예약 45 `tunnel_interfaces`를 전달하고 §72.2·H′를 이어간다.
...
- 🔴 **예약 46 전제** — `dmesg_coverage_check.py`의 rc=0만으로 커널 무사고를 확정하지
  않는다. 취득 시각 원본성이 없으면 rc=2이며, 사건 부재와 관측 창 부재를 분리한다.

## 진행표

| 트랙 | 묶음/근거 | 상태 |
|---|---|---|
| **진행 중 묶음** | §77 기록·코퍼스 이관 | ✅ 완료 · 다음은 Claude 보완 구현 |
| 1.6.2 현장 사다리 | ✅ 완료 · 구현자 주장 |
| 예약 41-g 링크 정지 | 🔴 사건별 계측 보완 전 미분류 |
| 예약 44 우전륜 유격 | 🔴 스페어 교체·동일 시행 재측정 전 상수 변경 금지 |
| 예약 46 dmesg 경계 | 🔴 취득 경계와 사건 경계 분리 전 무사고 증명 금지 |
