...
- **현재 단계**: 마스터플랜 5단 완료 + 순서 밖 소묶음 **e2e-harness-fix 완료** → 다음 = 마스터플랜
  **6단 역할 B V1 공동 합의** (`MASTER_PLAN.md §1` 6, 실차 7단과 **병렬 가능**).
...
## 이번 한 묶음 목표 — 역할 B와 `/detections` V1 최소 계약 공동 합의

`PROJECT_CONTEXT.md §4.1` 의 책임경계((b) camera-frame 3D)와 V1 필드 골격은 **역할 A 내부안**이며,
역할 B가 확인·동의한 상태가 아니다. 이번 묶음에서 책임경계와 다음 **세부 3종을 함께 합의**한다:
① 각 필드의 정확한 타입·단위·좌표계(특히 `position` 의 optical frame 축 방향과
...
2. **역할 B 협의 의존** — 책임경계·필드·주기·실패 표현 모두 역할 B 측 YOLO 파이프라인 능력에
   달려 있다. **역할 B의 명시 확인 전 `.msg`를 합의 완료로 고정하지 않는다.** 회의 전 초안은
   "역할 B 확인 필요" 상태로 유지한다.
...
1. 책임경계와 계약 세부 3종(필드 타입·단위·좌표계 / 주기·QoS / 실패·불확실 표현)을 역할 B가
   확인하고, 확인자·확인일과 함께 `~/Desktop/YOLO_탐지연동_합의사항.md` 에 기록한다. 회의 뒤에도
   미결인 항목은 **가정 + "역할 B 확인 필요"** 로 명시하되, V1 전체를 합의 완료로 선언하지 않는다.
...
**한 문장 완료판정**: "역할 B가 책임경계와 V1 계약 세부 3종을 명시 확인해 정본에 기록하고,
그 합의와 같은 `.msg`의 깡통 퍼블리셔→수신 funnel 왕복이 단위 테스트로 검증되며,
pytest·colcon 전량 통과한다."
