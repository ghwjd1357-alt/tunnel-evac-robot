...
## 직전 완료 — 검토 §37 보완 (구현자 = Codex, 독립 검토자 = Claude 예정)

Claude의 §37 독립 재검토는 `307759e`에서 P1 1·P2 3을 재현했다. 사용자 명시로 Codex가 이번
보완 diff의 구현자를 맡았으며, 자기 diff를 승인하지 않는다. 완료판정·전수 대조·검증 상한의
정본 기록은 `FREEZE_MANIFEST §10.12`다.

- 문서 소유 표식 두 쌍을 **정상 실측 기록 슬롯의 위치 계약**으로 삼았다. PASS/FAIL+시간·프레임
  실측과 회신 대조는 슬롯에서만 허용하고, 슬롯 밖·실측 생략·회신 단독·표식 소실은 계속 막는다.
- Markdown 표를 셀 단위로 분해하고 주어/판정을 이웃 두 문장으로 나눈 §37 재현형도 검사한다.
  동의어 전체·세 문장 이상·원거리 상호참조는 미폐포 상한으로 공개했다.
- §10.9가 열거한 실제 7문서 본문마다 6문형을 주입한다. 기존의 경로 무관 6종×7 반복 주장은
  취소했으며, 없는 활성 경로는 경로가 찍힌 계약 실패로 바꿨다.
- 검토 archive 규약을 `*검토현황.md` 접미사로 정확히 적고 §37 P1을 알려진 결함 62번째 행에
  편입했다. 규약 밖 이름이 자동 발견된다고 주장하지 않는다.

변경은 `docs/**`와 `tools/test_ai_context.py`·inventory뿐이다. `src/**`와 예약 22·23 구현은
건드리지 않았으며, 현재 단계는 계속 실차 D+0이다. 반대쪽 AI가 이 diff를 독립 검토하기 전에는
최종 승인으로 읽지 않는다.
