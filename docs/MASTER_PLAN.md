...
1. **SpeedManager 추출** — 구현 `f94da44` + 보완 **4회**. **완료 판정 = Codex 최종
   재검토 통과 시** (재검토마다 신규 P1 이 나와 §5 탈출조건 하에 연장 진행 중).
   기록 = `0720_현황.md §19` 구현 · `§20`·`§22`·`§23`·`§24` 검토·보완.
   P1 4건 중 3건이 "요청/과거 ≠ 현재 실효값"(§3-3) — §24 는 소비 지점 게이트의
   **술어가 stale latch** 였던 건(위치는 옳고 술어가 틀림)을 live 로 교정.
