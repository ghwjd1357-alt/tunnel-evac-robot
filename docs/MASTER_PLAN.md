...
4. **`abort_e2e` 진단 강화** — 실정지 단언 실패 시 `/cmd_vel` 잔류부터 읽어 "코드 결함 vs 잔류
   ...
   → **07-30 착수 (예약 16 과 한 묶음 = 'E2E 하네스 신뢰성')**. 구체 결함 지점: ⑦ 실패 시
   `fail()` 이 즉시 cleanup+exit 해(`tools/lib_e2e.sh:51`) ⑧ `/cmd_vel` 수집이 실행조차 안 되고
   노드까지 죽어 증거가 사라진다. 범위·완료조건 = `docs/CURRENT_HANDOFF.md`.
...
16. **`mission_e2e.sh` T자 간헐 실패 (미해소 — 관측만 등록)** — 07-30 전량 회귀 중 **2회 중 1회**
   ...
   → **07-30 착수 (예약 4 와 한 묶음)**. 이번 범위는 **규명까지** — 빈도 6회 이상 측정 +
   가설 ①(`visible` 미성립, `mission_node.py:690`) / ②(TF 조회 실패가
   `record_last_seen()` 의 `except Exception: pass` 에 삼켜짐, `mission_node.py:802`) 중 판정 +
   수리 위치 확정. 원인이 `mission_manager` 면 **동결 안**이라 고치지 않고 예외 요청을 올린다.
   범위·완료조건·임시 계측 규칙 = `docs/CURRENT_HANDOFF.md`.
