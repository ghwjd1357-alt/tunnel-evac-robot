...
10. **`tunnel_bringup` 게이트 회귀 하네스 · lint 편입** (07-29 등록) — `readiness_gate` 는 실차
   기동 순서를 좌우하는 신규 안전 로직인데 **자동 회귀가 없다**(07-29 검증은 전부 수동 실행).
   `tools/test_harness_guards.sh` 처럼 Gazebo 없이 도는 격리 하네스 + `tunnel_bringup` 을
   `colcon test` 대상에 편입(현재 lint 테스트 미포함 — 기준선 165 를 유지하려고 이번 묶음에서
   제외했다)이 필요하다. ⚠ 편입 시 `docs/TEST_GATES.md §1` 기준선 수치를 함께 갱신할 것.
