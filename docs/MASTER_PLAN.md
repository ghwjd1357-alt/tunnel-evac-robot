...
9. **`tunnel_bringup` S4 골격** — 착수 조건이 "트리거 임박 시"였으나 **트리거 시점을 역할 A 가 알 수
   없어 만족 불가능한 조건**이었다(07-29 확인). 실차·Jetson·역할 B 가 모두 대기라 지금이 유일한
   여유이며, 시뮬 자산 무변경이라 동결 위반이 아니다 → **순서 밖 소묶음으로 착수**(현재 핸드오프).
