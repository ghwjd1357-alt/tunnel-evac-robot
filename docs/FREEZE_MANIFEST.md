...
### 8.1 ✅ 예약 6·7 수리 완료 (07-24 e2e-harness-fix — 별도 커밋)

두 하네스 결함을 후속 묶음에서 고쳤다. **동결 런타임 코드는 무변경** — `tools/mission_e2e.sh`·
`tools/lib_e2e.sh`(테스트 하네스)만 손댔고, 동결 기준점·태그는 재개방하지 않는다.

| 결함 | 수리 | 검증 |
|---|---|---|
| ⑦ `ros2 param get` 무한 행 | `read_param_float`(`timeout 8` + `ros2 daemon` 재시작 1회, 상한 ≈19s). `-w 1` 토픽 pub 3종도 같은 블록 위험이라 `timeout` 예방 가드 | 스윕 결과 관측된 무한 행은 이 param get 이 유일. daemon·launch·run·`state`·readiness 는 이미 유한 |
| ⑧-a 폴링 race | `wait_state` 의 타임아웃 판정과 '마지막 상태' 보고를 **같은 읽기**로 통일 (독립 재읽기 제거) | "타임아웃인데 마지막 상태는 목표" 자기모순이 구조적으로 불가능 |
| ⑧-b 90s 예산 | **180s 로 재산정** (관측 최악 ≈90s 의 2배 마진 — 분포·근거 = `TEST_GATES.md §2`) | 신규 7회 SEARCH_BACK 전부 9s + 부정 회귀(도달 불가 시 FAIL) 실측 |
...
옛 90s 기준**으로 얻은 것이다. 수리 후 현재 기준은 **180s** 이며, 두 기준이 조용히 섞이지 않도록 여기
못 박는다. **동결 기준점·태그는 재개방하지 않는다** — 태그는 "그 시점 옛 기준으로 이런 수치였다"는
불변 기록이고, 새 기준은 이 별도 묶음부터 적용된다.

**재검증 실측** (07-24, 표준환경, 새 180s 기준):
- pytest **159 passed** / colcon **165 tests, 0 errors, 0 failures, 2 skipped** (무변동 — 셸 도구만 변경).
- `mission_e2e` (T자) **PASS** ×2, `mission_e2e twin` (쌍굴) **PASS** ×2 연속 — SEARCH_BACK 도달 전부 9s.
- 부정 회귀: `T_SEARCHBACK=3` → `FAIL: SEARCH_BACK 대기 타임아웃(3s), 마지막 상태='GUIDE'` (비모순).
