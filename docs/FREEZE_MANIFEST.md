...
### 10.30-a. 아홉 번째 예외 **범위 확장** — 검토 §83 반영 (2026-08-21, 같은 날 밤)

| | |
|---|---|
| 승인 | 사용자, 2026-08-21 (*"위 계획대로 진행하고 커밋 푸쉬"*) |
| 추가 범위 | `mission_node.py` — `State.BLOCKED` 신설 + `on_alarm` 거부 경로의 goal 취소 |
| 왜 새 예외를 안 열었나 | **같은 결함(§82.7 → §83.6)의 완결**이다. §10.30 이 연 자리에서 "거부한다" 를 실제 전이로 만드는 것이라 새 표면을 열지 않는다 |
| 변경 안 함 (경계) | 상태 전이 나머지 전량 · `follower_monitor.py` · `speed_manager` · Nav2 설정 · 런치(§10.30 ①에서 이미 확정) |
| 회귀 | `pytest src/mission_manager` **194**(신규 10) · S1-3 계약 재작성 |

**무엇이 바뀌었나** — §82.7 은 안전 집결지가 없으면 경고 후 `return` 했다. 재현 결과
`state=PATROL` · `cancel_current_goal()` 0회 · siren False 였다. 즉 **로봇이 기존 순찰
goal 을 그대로 계속 수행**했고, 정본이 적은 "관제로 넘긴다" 는 문장만이었다.

```
State.BLOCKED   자동 재시도 없음. tick 에 goal 을 내는 가지가 없다 → 로봇이 선다.
                탈출 = 관제 `reset` 뿐. 그것이 "사람에게 넘긴다" 의 실제 구현이다.
FAULT 와 다름   FAULT 는 Nav2 실패이고 MAX_RETRIES 자동 재시도가 있다.
                BLOCKED 는 계산이 안전 조건을 못 채운 것이라 재시도할 대상이 없다.
```

### 🔴 이 확장이 덮지 않는 것

- **대체 탈출구·집결지 재탐색은 안 만들었다.** 검토도 *"지금 안 만든 것은 PASS"* 로
  판정했다. 촬영 후 정책이다.
