...
- **전제조건**: R1·R2 저속 사다리 한정 · 사람이 E-stop 을 들고 옆에(`ELECTRICAL_BASELINE §7-a`①)
  ...
  · 🔴 **R1 지면 주행은 그대로 금지**다.
- 🔴 **닫기 전 남은 셋**: ⓵ `600ms` 를 고른 **사용자 근거 한 줄**(비어 있다 — 이 값은 최악
  지연의 증명이 아니라 **수용한 운용 상한**이다) ⓶ **현행 펌웨어로 영상 + bag 동시 재측정 1회**
  ⓷ **검토자 재확인**.
- 🔴 **재개방**: ⓵ 검토자가 ⓐ 정식화를 불승인 **(발동함 — §57)** ⓶ 현행 펌웨어 재측정이 새
  기준(총 정지 ≤600ms)을 넘음 ⓷ 자율 발행자가 실차 `/cmd_vel` 에 붙기 전 ⓸ 속도 상한을
  `0.12 m/s` 위로 올릴 때 (정지 거리 예산이 바뀐다).
