...
| §39 발견 | 표면 | 이번 회차 처리 |
|---|---|---|
| **P1 §39.2** 스냅샷 판정 2자리가 벽시계를 재지 않고 **상한 초를 사실로 단정**한다 | `tools/d0_check.sh` | ❌ 예약 25 (코드) |
| **P1 §39.6.1** §10 '다음 단계'가 모터 명령 경로를 열어 둔 채 낡았다 | `docs/JETSON_SETUP.md` | ✅ 닫음 |
| P2-1 `hard_timeout 8` 하드코딩과 `--secs` 분리 | `tools/d0_check.sh` | ❌ 예약 25 |
| P2-2 udev 주석이 규칙의 실제 선택도를 잘못 서술 | `tools/udev/**` | ❌ 예약 25 |
...
못 받았다"`를 출력하며(`:300`), 같은 결함이 `:552`의 `"5초 주기 × 12초 대기했다"`에도 있다.
관측 창 3자리(`226`·`234`·`238`)는 `elapsed_ms`로 이미 고쳐졌고 **스냅샷 2자리만 안 받았다**.
...
⚠ **이 절은 판정이 아니라 미해결 인벤토리다.** 예약 25가 닫히기 전까지 `c397975`와 그 후속
현장 기록을 "독립 승인된 완료판"으로 인용하지 않는다.
