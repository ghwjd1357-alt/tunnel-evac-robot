...
가 내려졌고(08-11 사용자), **검토자 확인**과 **현행 펌웨어 재측정**이 남았다.

- **전제조건**: R1·R2 저속 사다리 한정 · 사람이 E-stop 을 들고 옆에(`ELECTRICAL_BASELINE §7-a`①)
  · 자율 발행자를 `/cmd_vel` 에 붙이지 않는다(붙는 시점 = `§56.1` 재개방 1순위와 같은 자리).
- 🔴 **재개방**: ⓵ 검토자가 ⓐ 정식화를 불승인 ⓶ 현행 펌웨어 재측정이 새 기준(총 정지 ≤600ms)을
  넘음 ⓷ 자율 발행자가 실차 `/cmd_vel` 에 붙기 전 ⓸ 속도 상한을 `0.12 m/s` 위로 올릴 때
  (정지 거리 예산이 바뀐다).
...
**정본 측정은 bag**(`tools/watchdog_report.py`. 시작점이 실제 메시지 타임스탬프라 편향이 없다)
이고, **영상**(`tools/watchdog_video.py`)은 *펌웨어와 독립된* 1차 증거이되 터미널 렌더 지연만큼
**하한만 준다**(실측 49.5ms). EMA `/odom.twist`의 0 도달 시각은 판정에서 제외한다.
