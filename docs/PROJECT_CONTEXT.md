...
역할 A 회신 = `YOLO_탐지연동_역할A_5차회신_0819.md` (위 3·4·5·6 에 한 번에 답).
...
| N9 확정 지연 | 0.4초(가정) → 0.20초 → **≈0.48초**(8.4Hz, 잠정). 🔴 **어느 값도 계약값이 아니다** |
...
| # | 항목 | 상태 |
|---|---|---|
| **A10** | `tunnel_interfaces` tag + 40자 SHA | 🔶 노트북 빌드 완료 · **Jetson 확인·태그·전달 남음** (`MASTER_PLAN §7` 예약 45) |
| **A11** | 🔴 **URDF 카메라 링크 + `base_link↔camera` static TF** | 🔴 **신규** — `robot_real.urdf` 에 카메라 링크가 **0개**라 역할 B의 E3 질문에 답할 수 없다. 카메라 물리 장착 시 |
| **A12** | Jetson 잔여 자원 측정 (YOLO 제외 스택) | 🔴 신규 — 08-20 예정 |

⚠ **역할 B 회신 대기 6건** — T1~T4(기존) + **U1** `angle >= 120°` 프레임을 `valid` 에서
빼는지 명시 + **U2** 미발행 전환 동의. 🔴 **여섯 건 모두 시연을 막지 않는다.**
