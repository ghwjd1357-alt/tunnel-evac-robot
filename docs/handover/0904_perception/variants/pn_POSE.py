...
class PerceptionNode(Node):
    ...
    def __init__(self):
        ...
        # [실험] TensorRT 엔진으로 교체 — 파이토치 모델은 올리지 않는다
        import os as _os
        ...
        # [실험] TRT 엔진이라 optimize_for_inference 불필요
    ...
    def image_callback(self, color_msg: Image, depth_msg: Image) -> None:
        ...
        if ((self._fc - 1) % _every == 0) or (not hasattr(self, '_fire_cache')):
            try:
        ...
        try:
            ...
            # [빠른개선 A] 1.5배 확대 제거 — 뷰어에서 확대하면 된다
            debug_msg = Image()
    ...
    def _infer_fire(self, frame) -> List[Tuple[str, float, Tuple]]:
        ...
        # [빠른개선 B] 4번 훑던 것을 곱셈 1번으로. 결과값은 수학적으로 동일하다.
    ...
    def _infer_pose(self, frame):
        ...
        # [진단] ByteTrack 비용 분리 — track() 대신 predict(). track_id 는 순번으로 대체.
