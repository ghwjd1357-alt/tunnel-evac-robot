...
### 4-c-5. 2026-08-18 — 1.6.3 계측 굽기 (🔴 **보드 이력이지 승인 기록이 아니다**)

예약 41-g 계약 4판대로 사건 계측을 넣었다. 안전 경로(`rearm_gate.h` 전이·
`drive_wiring.h` 정지·PWM 출력단)는 **바이트 동일**이고, 바뀐 것은 계측 추가뿐이다.

| 항목 | 1.6.3 현장 후보 |
|---|---:|
| `version` | `rearm-latch-pi-link-probe-1.6.3` |
| `build` (굽힘 확정값) | **`Aug 18 2026 12:32:35`** |
| FLASH code / data / headers | **297,120 / 86,500 / 8,568** (1.6.2 대비 +1,856 / 0 / +192) |
| RAM1 variables / code | **63,616 / 162,104** · 지역변수 여유 **296,832** (−832) |
| RAM2 variables | **12,448** (불변) |
| `.hex` sha256 | `c40880ad277f7194113be1fdc434db18016f4bd49b58a50f2bd98204141de36e` |
| source sha256 (`.ino`) | `b238f79749526077796511aef13421c16d5847c740316423e5329f1536b966e9` (🔴 **승인값 아님**) |
| 되돌릴 산출물 | `~/d0_evidence/fw163/` (`.hex`·`.elf`) |
| 보드 관측 | `publish_max_us` **9칸** · `phase_max_us` **7칸** · `/firmware/pulse` 9.8Hz |

⚠ **증분이 설명되는가** — `§5-a` 의 *"1KB 이상 차이나면 중단"* 은 **무변경 재빌드**에서
환경 드리프트를 보는 규칙이다. 이번은 계측을 일부러 넣은 판이라 +1,856B 는 설명되는
증분이고, **증분 빌드와 clean 빌드가 바이트까지 같은 값**을 냈다(재현성 확인).

⚠ `/firmware/info` 상한이 1,407 → **1,418** 자가 되어 여유가 128 미만으로 떨어졌다
(`firmware_info_length_check.py` rc=1). 버퍼를 1,536 → **1,664** 로 올려 여유 **245** 로
복구했다. 🔴 이건 *"info 에 ring 을 싣는 것"* 이 아니라 계약이 요구한
**`publish_max_us` 9번째 칸** 하나다 — 생존 표본은 여전히 `/firmware/pulse` 로 따로 나간다.

🔴 **굽기 뒤 검증은 `§7-c-E` 13행 전량으로 했다.** 축소판 6행은
`rearm_field_wiring.py` 의 기대 해시가 08-17 이후 안 옮겨져 rc=2 로 거부됐다
(예약 47). 🔴 **그 해시를 지금 옮기지 않는다** — 도구가 스스로 적은 갱신 규칙이
*"13행 전량을 밟은 뒤 그 결과로 옮긴다"* 라, 건너뛰려고 옮기면 순환이다.

🔴 **이 절은 승인 기록이 아니다.** `§4` 의 허용 지문은 **1.6.1 에 그대로 둔다.**
1.6.3 의 독립검토 판정 전에는 위 source hash 를 허용값으로 승격하지 않으며,
다음 굽기도 precheck 를 우회하지 않는다.
