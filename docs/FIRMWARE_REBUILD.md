...
셋을 더 놓치고 있었다: **① 미추적 파일**은 `git diff` 에 아예 안 보이는데 `arduino-cli` 는 스케치
폴더의 소스를 전부 이어 붙여 굽고, **② `.h`·`.cpp`** 도 같이 굽히므로 `.ino` 만 보면
`motor_override.h` 한 장에 뚫리며, **③ 삭제·이름변경**은 기대 목록과 양방향으로 대조해야 잡힌다.
→ 이제 판정 입력은 **기준점 → 작업 트리**(커밋·staged·unstaged 전부) + **미추적 소스**이고,
판정은 사람 눈이 아니라 **종료코드**다. 🔴 **`2` 를 `0` 처럼 읽지 않는다** — "못 봤다"와
"봤는데 깨끗하다"는 다른 사실이다.
...
본다. 회귀 = `bash tools/test_firmware_precheck.sh` (**17 케이스** = 부정 11 · 역회귀 4 ·
구판 결함 박제 2).
