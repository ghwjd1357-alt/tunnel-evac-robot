...
|---|---|---|
| 첫 직진 안전 공간 | 오픈루프 횡편차를 모르므로 평지에서 **양옆 1m 이상** 확보 | `TODO(D+1): 확인` — ⚠ 08-11 잠정 **횡편차 1.8%**(12.4/676mm · IMU 교차). 🔴 **그 시행이 명령의 2.6배 속도였으므로 교정 후 다시 잰다**. `r1_ground_0811_2127` |
| 최고속 0.12m/s | 3m 실주행의 `/odom.header.stamp` 차로 평균속도 계산. PWM 천장 때문에 미달 가능 | `TODO(D+1): 확인` — 🔴 **08-11 실측 `0.3265 m/s` = 미달이 아니라 `2.7배 초과`**(3.000m/9.47s · `d0_drive_0811_2146`). 원인·교정값 = `MASTER_PLAN §7` 예약 32. 🔴 **교정 후 재측정 전까지 이 칸은 닫지 않는다 · 지면 주행 금지** |
...
| bag 실측 | **516.0 ms** (펌웨어 발동 500ms 뒤 +16.0ms · 운용 수용선 600ms 이내) · 민감도 `2mm/s→536.7` `5→516.0` `10→495.0` `20→473.8` |
...
- 재계산 = `python3 tools/watchdog_video.py <영상> --t0-frame 473 --preset 0811-1938
  --pre-plate --bag-ms 516.0` — 🔴 **판재 이전** 시행이라 반지름 profile 이 `pre-plate`
  여야 위 수가 재현된다(검토 §66.1). preset 이 스스로 선언하므로 잊어도 옳게 나온다.
...
- 재계산 = `python3 tools/watchdog_video.py <영상> --t0-frame 670 --preset 0807-1522
  --pre-plate --bag-ms 516.2` — 🔴 **판재 이전** profile (§66.1)
