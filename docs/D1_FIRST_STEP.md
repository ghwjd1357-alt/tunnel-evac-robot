...
| R0 watchdog | 명령 단절 뒤 실제 바퀴 정지. EMA twist가 아니라 영상(1차)+raw pose(정본 측정)로 판정 | 🔴 **미충족 — 실측은 끝났고 검토자 재확인이 남았다** (`JETSON_SETUP §7-c-0`) |
...
| PASS/FAIL | 🔴 **판정 불능** — 영상은 PASS 를 못 만들고, ⓐ 정식화는 검토 확인 보류다. **1-b `≤600ms` 는 bag 실측으로 충족** |
...
- 🔴 **영상 분석 구간을 `413~650` 으로 끊었다** — `651` 이후는 손각대 흔들림이 잡음 바닥을
  `1.089°` 로 올려 판정선 `1.006°` 를 넘겼고 **전 구간은 판정 불능**이다. 끊은 구간이
...
- 🔴 **`#11` 은 아직 열려 있다** — 남은 것은 **검토자 재확인** 하나이고, 그 전까지
  **R1 지면 주행 금지**는 유지다.
...
- 🔴 **`#11` 은 열려 있다.** 결정 1 = 안 ⓐ(기준 재정의)가 08-11 에 내려졌으나 구현자 정식화는
  ...
  ✅ **재측정은 08-11 19:41 에 끝났다**(위 블록). 남은 것 = **검토자 재확인** 하나.
